...
class UserMapper(BaseMapper[UserDTO]):
    ...
    def convertFromJsonToDto(self, json: Dict) -> UserDTO:
        return UserDTO(userName=json.get("username"), role=json.get("role"))

    @classmethod
    def convertFromDtoToJson(userDto: UserDTO) -> Dict:
        return {
            "id": userDto.id,
            "username": userDto.userName,
            "role": userDto.role,
            "created_at": (
                userDto.created_at.isoformat() if userDto.created_at else None
            ),
        }
